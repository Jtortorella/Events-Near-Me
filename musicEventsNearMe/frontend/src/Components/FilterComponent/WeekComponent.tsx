...
export const WeekComponent = () => {
    ...
    if (event) {
      setFilter((prev: Filter) => {
        if (filter.startDate && !filter.endDate) {
          ...
          const startDate = compareDates(filter.startDate, event) >= 0
            ? new Date(filter.startDate)
            : new Date(event);
          startDate.setHours(0, 0, 0);
  
          const endDate = compareDates(filter.startDate, event) >= 0
            ? new Date(event)
            : new Date(filter.startDate);
          ...
          const startDate = compareDates(event, filter.endDate) === -1
            ? new Date(filter.startDate)
            : new Date(event);  
          const endDate = compareDates(event, filter.endDate) === -1
            ? new Date(event)
  ...
  return (
    <div className="week-container">
      <button className="week-selector" onClick={decrementWeek}>&lt;</button>
      {generateWeek()}
      <button className="week-selector" onClick={incrementWeek}>&gt;</button>
    </div>
  );
